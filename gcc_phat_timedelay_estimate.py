...
class timeDelayEstimate:
    def __init__(self):
        ...
        # 滤波器系数
        self.b1 = np.array([0.97261, -1.94523, 0.97261])
        self.a1 = np.array([-1.94448, 0.94598])
        self.b_m = np.array(
    ...
    def Update(self, capture):
        ...
        for n in range(self.filter_.shape[0]):
            ...
            self.acc_index += 1
            #print(f"render buffer:{self.render_buffer[2431]}")
            #print(f"render_buffer_read_index:{self.render_buffer_read_index}")
            #print(f"index:{self.acc_index},accuracy:{round(error_sum_anchor - self.error_sum, 5)}")
            if flag:
                #print(f"index:{self.acc_index},accuracy:{round(error_sum_anchor - self.error_sum, 5)}")
    ...
    def ComputeBufferDelay(self, hysteresis_limit_blocks):
        ...
        print(f"new delay blocks:{new_delay_blocks}")
...
def gcc_phat(sig, ref):
    ...
    for n in range(num_block-1):
        if tde.capture_properly_started_ == False:
            ...
            x_n = sig[n * M:(n + 1) * M]
            x_n = tde.SigFilterProcess(x_n)
            timedelay[n] = tde.EstimateDelay(x_n)
